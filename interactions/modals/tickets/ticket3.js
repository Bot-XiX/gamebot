...
/**
 * @file Modal interaction: ticket3
 * @since 1.0.0
*/
module.exports = {
  id: 'ticket3',
  /**
* @description Executes when the modal with ID ticket3 is called.
  ...
  async execute (interaction) {
    ...
    const unsub = onValue(config, async (snapshot) => {
      ...
      try {
        ...
        const amount = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/3/modals/amount')).then((snapshot) => snapshot.val())
        for (let i = 0; i < amount; i++) {
          const modalTitle = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/3/modals/' + i + '/name'))
          // Get
