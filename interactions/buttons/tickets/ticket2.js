...
*/
  async execute (interaction) {
    try {
      ...
      const modalsEnabled = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/2/modals/enabled')).then((snapshot) => snapshot.val())
      if (modalsEnabled) {
        const modalTitle = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/2/modals/title')).then((snapshot) => snapshot.val())
        const modalModal = new ModalBuilder().setCustomId('ticket2').setTitle(modalTitle)
        const modalAmount = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/2/modals/amount')).then((snapshot) => snapshot.val())
        const modalRows = []
        for (let i = 0; i < modalAmount; i++) {
          const modalRow = new ActionRowBuilder()
          const modalText = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/2/modals/' + i + '/name')).then((snapshot) => snapshot.val())
          const modalPlaceholder = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/2/modals/' + i + '/placeholder')).then((snapshot) => snapshot.val())
          const modalType = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/2/modals/' + i + '/type')).then((snapshot) => snapshot.val())
          const modalRequired = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/2/modals/' + i + '/required')).then((snapshot) => snapshot.val())
