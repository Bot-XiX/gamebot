...
*/
  async execute (interaction) {
    try {
      ...
      const modalsEnabled = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/1/modals/enabled')).then((snapshot) => snapshot.val())
      console.log(modalsEnabled)
      if (modalsEnabled) {
        const modalTitle = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/1/modals/title')).then((snapshot) => snapshot.val())
        const modalModal = new ModalBuilder().setCustomId('ticket1').setTitle(modalTitle)
        const modalAmount = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/1/modals/amount')).then((snapshot) => snapshot.val())
        const modalRows = []
        for (let i = 0; i < modalAmount; i++) {
          const modalRow = new ActionRowBuilder()
          const modalText = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/1/modals/' + i + '/name')).then((snapshot) => snapshot.val())
          const modalPlaceholder = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/1/modals/' + i + '/placeholder')).then((snapshot) => snapshot.val())
          const modalType = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/1/modals/' + i + '/type')).then((snapshot) => snapshot.val())
          const modalRequired = await get(ref(db, id + '/tickets/config/' + configID + '/buttons/components/1/modals/' + i + '/required')).then((snapshot) => snapshot.val())
