        ...
        if (user.user !== interaction.member) {
          try {
            ...
            if (member.voice.channel === channel) {
              if (channel.permissionsFor(user.id).has(PermissionsBitField.Flags.KickMembers)) return interaction.editReply({ content: `${user.toString()} kann nicht gebannt werden!`, ephemeral: true })
