...
/**
 * @file Button interaction: customVoiceRemoveFriend
 * @since 1.0.0
*/
module.exports = {
  id: 'customVoiceRemoveFriend',
  /**
  * @description Executes when the button with ID customVoiceRemoveFriend is called.
  ...
  */
  async execute (interaction) {
    ...
    interaction.editReply({ content: 'Bitte erwähne den User, den du als Freund entfernen möchtest!', components: [disabledRow], ephemeral: true })
    ...
    if (msg.size !== 0) {
      ...
      if (user) {
        if (friends.includes(user.id)) {
          const index = friends.indexOf(user.id)
          if (index > -1) {
            friends = friends.filter((friend) => friend !== user.id)
            set(ref(db, `users/${interaction.user.id}/friends`), friends)
          }
          interaction.editReply({ content: `${user} wurde als Freund entfernt!`, ephemeral: true })
        } else {
          interaction.editReply({ content: `${user} ist nicht dein Freund!`, ephemeral: true })
        }
      }
    } else {
